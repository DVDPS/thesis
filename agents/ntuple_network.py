...
class NTupleNetwork:
    ...
    def evaluate(self, state: np.ndarray) -> float:
        ...
        for idx, tuple_indices in enumerate(self.n_tuples):
            ...
            feature = tuple(flat_state[i] for i in tuple_indices)
            # If feature not seen before, use optimistic initialization.
            w = self.weights[idx].get(feature, self.optimistic_value)
            total_value += w
        return total_value

    def update_weights(self, state: np.ndarray, td_error: float, learning_rate: float):
...
class OptimisticTDAgent:
    ...
    def update(self, state: np.ndarray, reward: float, next_state: np.ndarray, terminal: bool) -> float:
        """
        Perform a TD update:
